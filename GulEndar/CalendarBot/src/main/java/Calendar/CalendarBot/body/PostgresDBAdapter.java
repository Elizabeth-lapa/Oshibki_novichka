...
public class PostgresDBAdapter {
    ...
    public boolean createTable() throws SQLException {
        try(Connection DB_connection = DriverManager.getConnection(DB_URL,USER ,PASS )){
            try (Statement stmt = DB_connection.createStatement()) {
                ...
                String tableSql = "create table calendar.Event(chatID bigint , text text, datetime timestamp,duration integer);";
    ...
    public boolean addEvent(Long chatID, String text, java.time.LocalDateTime dateTime, int duration) throws SQLException {
        try(Connection DB_connection = DriverManager.getConnection(DB_URL,USER ,PASS )){
            try (Statement stmt = DB_connection.createStatement()) {
            ...
            try (Statement stmt = DB_connection.createStatement()) {
                ...
                while(res.next()){
                    for (int i = 1; i <= columns; i++){
                        ...
                        eventsList.add(new Event(res.getString(1),LocalDateTime.parse(res.getString(2), formatter),res.getInt(3)));
                    }
                    }
                res.close();
                return eventsList;
